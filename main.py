...
while True:
    try:
        sound_device = sd.query_devices(device=index_sound)
        if sound_device:
            if sound_device['max_input_channels'] > 0:
                sounddevices.append(sound_device)
                print(f"Input Device {index_sound}: {sound_device['name']}")
            elif sound_device['max_output_channels'] > 0:
                print(f"Output Device {index_sound}: {sound_device['name']}")
            index_sound += 1
        else:
            break
    except sd.PortAudioError as e:
        print(f"Error querying device {index_sound}: {e}")
        break

print("All Sound Devices:")
for index, device in enumerate(sounddevices):
    print(f"{index + 1}. {device['name']} - {device['hostapi']} - {device['default_samplerate']} Hz")



thesounddevices = []
for i in range(len(sounddevices)):
    device_info = sounddevices[i]
    device_tuple = (device_info["index"], device_info["name"], device_info["default_samplerate"])
    thesounddevices.append(device_tuple)
...
class VideoRecorder:
    def __init__(self, root):
        ...
        self.options_button = tk.Button(root, image=iconopt, command=self.options, width=self.button_height, height=self.button_height)
        ...
        self.create_timeline_button = tk.Button(root, text="Import Video", font = ("Times",30), pady = self.button_height // 7.5,command=self.create_timeline)
        self.create_timeline_button.pack(side=tk.RIGHT, anchor=tk.S)


        self.stop_button = tk.Button(root, text="Stop", font=("Times",30), pady = self.button_height // 7.5, command=self.stop_recording,
                                     state=tk.DISABLED)
        self.stop_button.pack(side=tk.RIGHT, anchor=tk.S)

        self.listen_button = tk.Button(root, text="Listen to Start Gun", font = ("Times",30), pady = self.button_height // 7.5, command=self.listen_start_gun)
    ...
    def result(self):
        ...
        def display_results(number_of_athletes = 0):
            ...
            for i in range(number_of_athletes,len(self.athletes)):
                ...
                X_license.insert(0, self.athletes[i]["priv_id"])
                ...
                X_editbutton = tk.Button(self.result_window,image = editpic,command= lambda: edit_athlete(no = X_private_id))
                X_editbutton.place(x = 6+740, y = 5+20+i*20, width = 20, height = 20)

                X_deletebutton = tk.Button(self.result_window, image=crosspic, command=lambda : delete_athlete(no = X_private_id))
                X_deletebutton.place(x=6 + 760, y=5+20 + i * 20, width=20, height=20)

                X = [X_lane,X_id,X_name,X_affiliation,X_license,X_time,X_place,X_editbutton,X_deletebutton,X_private_id]

                self.resultlabels.append(X)
                print(self.resultlabels[i][-1])
                print(self.athletes[i])
            self.add_athlete_button.place(x=370, y=20 + len(self.athletes) * 20 + 20)
        display_results(len(self.athletes))
    ...
    def options(self):
        ...
        align_camera_button = tk.Button(self.settings, text="Align Camera", font=("Arial", 14), command=self.align_camera)
                ...
                def audio_callback(indata, frames, time, status):
                    ...
                    if self.canvas and hasattr(self, 'oval'):
                        ...
                        try:
                            ...
                            self.maxoval = self.canvas.create_oval(self.xmax - 2, 215 - 2, self.xmax + 2, 215 + 2, fill="blue")
        ...
        def save_options():
            ...
            if is_camera_available(int(camera_device.get())) == True:
                ...
                tk.messagebox.showerror(title="Invalid Device Index", message="There is no capture device with index "+camera_device.get()+"!")
    ...
    def align_camera(self):
        ...
        def open_aligned_camera():
            ...
            align_label_widget.after(10, open_aligned_camera)
